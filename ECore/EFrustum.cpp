...
namespace E3D
{
	EFrustum::EFrustum(CAMERA_TYPE mode, const EVector4D &pos, const EVector4D &dir,
		...
		:camMode(mode), position(pos), direction(dir), campTarget(target),
		clip_z_near(nearZ), clip_z_far(farZ), fov(ffov),
		viewport_width(viewportWidth), viewport_height(viewportHeight),
		camUp(EVector4D::UNIT_Y), campRight(EVector4D::UNIT_X), campLook(EVector4D::UNIT_Z),
		mWorldToCamera(EMatrix44::IDENTITY), mCameraToPerspective(EMatrix44::IDENTITY),
		mPerspectiveToScreen(EMatrix44::IDENTITY)
	{
		viewport_center_X = (viewport_width - 1)*0.5f;			//TODO
		viewport_center_Y = (viewport_height - 1)*0.5f;			//TODO

		aspect_ratio = viewport_width / viewport_height;
		viewplane_width = 2.0f;
		viewplane_height = 2.0f / aspect_ratio;

		EFloat tan_fox_div2 = tan(Degree_To_Radian(fov*0.5f));
		view_dist = 0.5f*viewplane_width / tan_fox_div2;
		...
		//右裁剪面
		EVector4D normalR = EVector4D(-view_dist, 0, viewplane_width*0.5f);		//TODO 为什么view_dist 是负的
		clip_plane_R = EPlane3D(point, normalR);

		//左裁剪面
		EVector4D normalL = EVector4D(view_dist, 0, viewplane_width*0.5f);		//TODO view_dist
		clip_plane_L = EPlane3D(point, normalL);

		//上裁剪面
		EVector4D normalT = EVector4D(0, -view_dist, viewplane_height*0.5f);		//TODO 
		clip_plane_T = EPlane3D(point, normalT);

		//下裁剪面
		EVector4D normalB = EVector4D(0, view_dist, viewplane_height*0.5f);		//TODO 
